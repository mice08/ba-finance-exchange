...
public class LogAspect {
    ...
    @AfterThrowing(value="execution(* com.dianping.ba.finance.exchange.biz.producer.ExchangeOrderStatusChangeNotify.*(..))",throwing="ex")
    public void afterThrowingRetry(JoinPoint joinPoint,Exception ex){
        LogUtils.log(monitorLogger, startTime, joinPoint.getTarget().toString()+"."+joinPoint.getSignature().getName(), Level.ERROR, Arrays.toString(joinPoint.getArgs()), ex);
    }
}
